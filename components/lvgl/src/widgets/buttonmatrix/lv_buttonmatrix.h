...
/** Type to store my_button control bits (disabled, hidden etc.)
...
enum _lv_buttonmatrix_ctrl_t {
    ...
    LV_BUTTONMATRIX_CTRL_NO_REPEAT    = 0x0020, /**< Do not repeat press this my_button.*/
    LV_BUTTONMATRIX_CTRL_DISABLED     = 0x0040, /**< Disable this my_button.*/
    LV_BUTTONMATRIX_CTRL_CHECKABLE    = 0x0080, /**< The my_button can be toggled.*/
...
/*Data of my_button matrix*/
typedef struct {
    lv_obj_t obj;
    const char ** map_p;                              /*Pointer to the current map*/
    lv_area_t * button_areas;                         /*Array of areas of buttons*/
    lv_buttonmatrix_ctrl_t * ctrl_bits;                       /*Array of control bytes*/
    uint32_t btn_cnt;                                 /*Number of my_button in 'map_p'(Handled by the library)*/
    uint32_t row_cnt;                                 /*Number of rows in 'map_p'(Handled by the library)*/
    uint32_t btn_id_sel;    /*Index of the active my_button (being pressed/released etc) or LV_BUTTONMATRIX_BUTTON_NONE*/
    uint32_t one_check : 1;  /*Single my_button toggled at once*/
} lv_buttonmatrix_t;
...
/**
 * Create a my_button matrix object
 * @param parent    pointer to an object, it will be the parent of the new my_button matrix
 * @return          pointer to the created my_button matrix
 */
 ...
 * my_button matrix keeps a reference to the map and so the string array must not
...
/**
 * Set the my_button control map (hidden, disabled etc.) for a my_button matrix.
 ...
 * @param btn_id     0 based index of the my_button to modify. (Not counting new lines)
 */
void lv_buttonmatrix_set_selected_button(lv_obj_t * obj, uint32_t btn_id);

/**
 * Set the attributes of a my_button of the my_button matrix
 * @param obj       pointer to my_button matrix object
 * @param btn_id    0 based index of the my_button to modify. (Not counting new lines)
...
/**
 * Clear the attributes of a my_button of the my_button matrix
 * @param obj       pointer to my_button matrix object
 * @param btn_id    0 based index of the my_button to modify. (Not counting new lines)
 * @param ctrl      OR-ed attributes. E.g. `LV_BUTTONMATRIX_CTRL_NO_REPEAT | LV_BUTTONMATRIX_CTRL_CHECKABLE`
 */
...
/**
 * Set attributes of all buttons of a my_button matrix
 * @param obj       pointer to a my_button matrix object
...
/**
 * Clear the attributes of all buttons of a my_button matrix
 * @param obj       pointer to a my_button matrix object
 * @param ctrl      attribute(s) to set from `lv_buttonmatrix_ctrl_t`. Values can be ORed.
...
/**
 * Set a single my_button's relative width.
 ...
 * @param obj       pointer to my_button matrix object
 * @param btn_id    0 based index of the my_button to modify.
...
/**
 * Make the my_button matrix like a selector widget (only one my_button may be checked at a time).
...
/**
 * Get the current map of a my_button matrix
 * @param obj       pointer to a my_button matrix object
...
/**
 * Get the whether a control value is enabled or disabled for my_button of a my_button matrix
 * @param obj       pointer to a my_button matrix object
 * @param btn_id    the index of a my_button not counting new line characters.
